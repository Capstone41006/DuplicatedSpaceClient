...
public static class UltiDraw {
	...
	static void Initialise() {
        ...
        Shader shader = Shader.Find("UltiDraw");
        if (shader == null)
        {
            Debug.LogWarning("Shader 'UltiDraw' not found. Using default shader.");
            shader = Shader.Find("Standard"); // 기본 셰이더로 대체
        }
        if (shader != null)
        {
        ...
        {
            Debug.LogError("No shader found. Material initialization failed.");
        }
    ...
    //private static Mesh GetPrimitiveMesh(PrimitiveType type) {			// 이 코드에서 Resource 파일을 인식 못해서 대체함
    //	return Resources.Load<GameObject>(type.ToString()+"Mesh").GetComponent<MeshFilter>().sharedMesh;
    ...
    {
        string resourcePath = type.ToString() + "Mesh";
        GameObject prefab = Resources.Load<GameObject>(resourcePath);
        if (prefab == null)
        {
            Debug.LogWarning($"Resource not found: {resourcePath}. Creating default primitive.");
        ...
        if (meshFilter == null)
        {
            Debug.LogError($"MeshFilter not found in prefab: {resourcePath}");
            return null;
        }
